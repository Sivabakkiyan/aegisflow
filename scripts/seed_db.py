...
DSN = (
    ...
    f"dbname={os.getenv('DB_NAME', 'aegisflow')} "
    f"user={os.getenv('DB_USER', 'aegis')} "
...
def seed_rules(cur) -> None:
    cur.executemany(
        ...
        "(rule_code, version, description, weight, params) "
...
def seed_watchlist(cur) -> None:
    cur.executemany(
        ...
        "(list_name, entity_name, entity_type, country_code) "
        "VALUES (%s, %s, %s, %s)",
        WATCHLIST,
    )
...
def seed_customers(cur, n: int = 120) -> list[str]:
    ...
    for i in range(n):
        cur.execute(
            ...
            "(external_ref, full_name, date_of_birth, country_code, "
            "kyc_status, risk_rating, is_pep) "
        ...
        for a in range(random.randint(1, 2)):
            cur.execute(
                ...
                "opened_on, last_active_on, status) "
                "VALUES (%s, %s, %s, %s, %s, %s, %s) "
...
def seed_transactions(
    ...
    cur.executemany(
        ...
        "(external_ref, account_id, channel, direction, amount, "
...
def main() -> None:
    ...
    print(
        "seeded: 6 rules, 3 watchlist entries, "
        "120 customers, ~180 accounts, 5000 transactions"
    )
